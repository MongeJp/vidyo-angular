/* Global VidyoClient objects */
var VC = {};
var VCUtils = {};

function VidyoClientInitialize($) {
  VCUtils.version = "4.1.23.21";
  VCUtils.versionWithoutBuildNumber = "4.1.23";
  VCUtils.mimeType = "application/x-vidyoclientplugin-" + VCUtils.version;
  /* jQuery local namespace */
  VCUtils.jQuery = $;
  /* parameters in VidyoClient.js */
  var search = $("script[src*='VidyoClient.js']").attr('src').split('?')[1];
  VCUtils.params = search ? $.parseJSON('{"' + decodeURI(search).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '"}') : {};
  var search = window.location.search.split('?')[1];
  VCUtils.windowParams = search ? $.parseJSON('{"' + decodeURI(search).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '"}') : {};

  /* Path used for retrieving other libraries */
  VCUtils.jsPath = $("script[src*='VidyoClient.js']").attr('src').split('javascript/VidyoClient/VidyoClient.js')[0];

  /* look for the callback */
  if (VCUtils.params.onload && window[VCUtils.params.onload]) {
    VCUtils.onloadCallback = window[VCUtils.params.onload];
  } else {
    return;
  }
  ...
  /* Load platform info */
  VCUtils.platformInfo = {};
  loadPlatformInfo(VCUtils.platformInfo);

  /* Check for browser support */
  if(VCUtils.platformInfo.isIE) {
    /* IE only supports activeX plugins */
    try{
      VCUtils.pluginAvailable = new ActiveXObject("Vidyo.VidyoClientPlugIn." + VCUtils.version);
    }catch(err){
      VCUtils.pluginAvailable = false;
    }
  } else {
    /* All other browsers */
    /* check for webRTC support */
    VCUtils.webRTCAvailable = VCUtils.platformInfo.isWebRTCAvailable;

    /* Global variable that stores the address of the WebRTC server */
    if (VCUtils.params.webrtcserver) {
      VCUtils.webRTCServer = VCUtils.params.webrtcserver;
    } else if (VCUtils.windowParams.webrtcserver) {
      VCUtils.webRTCServer = VCUtils.windowParams.webrtcserver;
    } else {
      VCUtils.webRTCServer = "https://webrtc.prod.vidyo.io";
    }

    /* check for plugin support */
    navigator.plugins.refresh(false);
    VCUtils.pluginAvailable = navigator.mimeTypes[VCUtils.mimeType];
  }

  VCUtils.electronAvailable = VCUtils.platformInfo.isElectron;

  var OnLoadSuccess_ =  function(){
    VCUtils.vcOnLoadComplete = true;
    $('#VidyoClientPlugIn').remove();
    $('body').append("<div id='VidyoClientPlugIn' style='position: absolute; visibility: hidden; width: 0px; height: 0px;'></div>");
    VC = new VidyoClientLib.VidyoClient("VidyoClientPlugIn", OnLoadedCallback_);
  }

  var OnLoadError_ = function(){
    if(!VCUtils.vcOnLoadComplete){
      OnLoadedCallback_({state: "FAILED", description: "Unable to load resources", type: "LOAD"});
    }
  }

  var OnLoadedCallback_ = function(status) {
    switch (status.state) {
      case "READY":
      ...
      case "NOTAVAILABLE": // The library is not available
        setTimeout($.proxy(OnReInitialize_, this), 3000);
        break;
    }

    if (VCUtils.onloadCallback) {
      GetDownloadLinks(status);
      VCUtils.onloadCallback(status);
    }
  }
  var OnReInitialize_ = function () {
    VidyoClientInitialize(VCUtils.jQuery);
  }

  var GetDownloadLinks = function(status) {
    var downloadPath = VCUtils.jsPath + "bin";
    var downloadPathApp = '';
    var downloadPathPlugIn = '';
    var i = VCUtils.platformInfo;
    ...
    if (i.isMac) {
      ...
      downloadPath += "/android";
      downloadPathApp = downloadPath + "/VidyoConnector.apk";
    } else if (i.isiOS) {
      downloadPathApp = "https://itunes.apple.com/us/app/vidyo-io-connector/id1322195285?mt=8";
    }

    if (!i.isMac && !i.isWin) {
      /* Mobile */
      downloadType = "MOBILE";
    } else if (i.isSafari || i.isFirefox || (i.isAppleWebKit && i.isMac) || (i.isIE && !i.isEdge)) {
      /* Supports Plugins */
      downloadType = "PLUGIN";
    } else {
      /* Supports App */
    ...
    status.downloadType = downloadType;
    status.downloadPathApp = downloadPathApp;
    status.downloadPathPlugIn = downloadPathPlugIn;

    /* Add WebRTC WindowShare extensions */
    if (i.isChrome) {
      status.downloadPathWebRTCExtensionChrome = "https://chrome.google.com/webstore/detail/vidyo-screen-share/fadjebjcpiiklefiadeicakcnkhgbaoo";
    } else if (i.isFirefox) {
      status.downloadPathWebRTCExtensionFirefox = VCUtils.webRTCServer + "/web/installers/vidyo_webrtc_screen_share-1.0.0-fx.xpi";
    }
  }

  if (VCUtils.electronAvailable) {
    /* load multiple js files at the same time */
    if (VCUtils.vcOnLoadComplete) {
      OnLoadSuccess_();
    } else {
      var es6 = document.createElement('script');
      es6.src = VCUtils.jsPath + "javascript/es6-promise/4.0.5/es6-promise.auto.min.js";
      var vcd = document.createElement('script');
      vcd.src = VCUtils.jsPath + "javascript/VidyoClient/VidyoClientDispatcher.js";
      var vct = document.createElement('script');
      vct.src = VCUtils.jsPath + "javascript/VidyoClient/VidyoClientTransportElectron.js";
      es6.onload = function(){
        vcd.onload = function(){
          vct.onload = function(){
            OnLoadSuccess_();
          }
          document.head.appendChild(vct);
        };
        document.head.appendChild(vcd);
      };
      document.head.appendChild(es6);
      setTimeout($.proxy(OnLoadError_, this), 4000);
    }

  } else if (VCUtils.params.plugin != "false" && VCUtils.windowParams.plugin != "false" && VCUtils.pluginAvailable) {
    /* load multiple js files at the same time */
    if (VCUtils.vcOnLoadComplete) {
      OnLoadSuccess_();
    } else {
      ...
      vcd.src = VCUtils.jsPath + "javascript/VidyoClient/VidyoClientDispatcher.js";
      var vct = document.createElement('script');
      vct.src = VCUtils.jsPath + "javascript/VidyoClient/VidyoClientTransportPlugIn.js";
      es6.onload = function(){
        vcd.onload = function(){
          vct.onload = function(){
            OnLoadSuccess_();
  ...
  } else if (VCUtils.params.webrtc != "false" && VCUtils.windowParams.webrtc != "false" && VCUtils.webRTCAvailable) {
    /* load multiple js files at the same time */
    if (VCUtils.vcOnLoadComplete) {
      OnLoadSuccess_();
    } else {
      /* Use of Native WebRTC has to be explicitly set for now */
      if (VCUtils.params.useNativeWebRTC == "true" || VCUtils.windowParams.useNativeWebRTC == "true") {
        /* add css into dom */
        $('<link/>', { rel: 'stylesheet', type: 'text/css', href: VCUtils.jsPath + 'javascript/VidyoClient/WebRTC/VidyoClient.css'}).appendTo('head');
        $.when(
          $.getScript(VCUtils.jsPath + "javascript/VidyoClient/WebRTC/VidyoClient.js"),
        ...
        $.when(
          /* Use of Native WebRTC has to be explicitly set for now */
          $.getScript(VCUtils.jsPath + "javascript/VidyoClient/VidyoClientDispatcher.js"),
          $.getScript(VCUtils.jsPath + "javascript/VidyoClient/VidyoClientTransportWebRTC.js"),
          $.Deferred(function(deferred){
            $(deferred.resolve);
          })
        ).done(function(){
  ...
  } else if (VCUtils.params.webserver == "true" || VCUtils.windowParams.webserver == "true") {
    /* load multiple js files at the same time */
    if (VCUtils.vcOnLoadComplete) {
      OnLoadSuccess_();
    } else {
      $.when(
        $.getScript(VCUtils.jsPath + "javascript/es6-promise/4.0.5/es6-promise.auto.min.js"),
        $.getScript(VCUtils.jsPath + "javascript/VidyoClient/VidyoClientDispatcher.js"),
        $.getScript(VCUtils.jsPath + "javascript/VidyoClient/VidyoClientTransportWeb.js"),
        $.Deferred(function(deferred){
          $(deferred.resolve);
        })
      ).done(function(){
    ...
    OnLoadedCallback_({state: "NOTAVAILABLE", description: "Not available", type: "LOAD"});
  }
}
      ...
      pseudos: {
        ...
        lang: ot(function(e) {
          return X.test(e || "") || st.error("unsupported lang: " + e), e = e.replace(et, tt).toLowerCase(), function(t) {
            var n;
            do if (n = d ? t.getAttribute("xml:lang") || t.getAttribute("lang") : t.lang)
...
VidyoClientInitialize($.noConflict(true));

//#sourceURL=VidyoClient.js
/* Global VidyoClient objects */
var VC = {};
var VCUtils = {};
